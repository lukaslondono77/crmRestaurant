(function () {
	...
	// Theme Settings
	// Dark/Light Toggle (only "light" / "dark" for full-page theme; other keys are for sidebar/header only)
	...
	if (getSwitchToggleId) {
		...
		const sidebarDark = localStorage.getItem("sidebar-dark-mode") === "true";
		// Apply saved theme: fila_theme takes precedence; fall back to sidebar-dark-mode
		if (savedTheme === "light" || savedTheme === "dark") {
			document.body.setAttribute("data-theme", savedTheme);
		} else if (sidebarDark) {
			document.body.setAttribute("data-theme", "dark");
		}
		getSwitchToggleId.addEventListener("click", function (e) {
			e.stopPropagation();
			const current = document.body.getAttribute("data-theme");
			if (current === "dark") {
				document.body.setAttribute("data-theme", "light");
				localStorage.setItem("fila_theme", "light");
				localStorage.setItem("sidebar-dark-mode", "false");
			} else {
				document.body.setAttribute("data-theme", "dark");
				localStorage.setItem("fila_theme", "dark");
				localStorage.setItem("sidebar-dark-mode", "true");
			}
		});
	}
...
// Fix for menu.js:520 error - Cannot read properties of undefined (reading 'destroy')
// This patch ensures manageScroll safely checks for scrollbar before destroying
(function() {
	"use strict";
	
	function patchMenuManageScroll() {
		// Wait for Menu class and mainMenu to be available
		if (typeof window.Helpers !== 'undefined' && window.Helpers.mainMenu) {
			const menu = window.Helpers.mainMenu;
			
			// Only patch if not already patched
			if (menu && !menu._manageScrollPatched) {
				const originalManageScroll = menu.manageScroll;
				
				if (originalManageScroll && typeof originalManageScroll === 'function') {
					menu.manageScroll = function() {
						try {
							...
							if (window.innerWidth < (window.Helpers?.LAYOUT_BREAKPOINT || 1200)) {
								// Mobile: destroy scrollbar if it exists
								...
								// Desktop: create scrollbar if it doesn't exist
								if (this._scrollbar === null && PerfectScrollbar) {
									try {
										...
										console.warn('Failed to create PerfectScrollbar:', err);
										this._scrollbar = null;
									}
								}
							...
							console.warn('Menu manageScroll error:', error);
					...
					menu._manageScrollPatched = true;
				}
			}
		}
	}
	...
	patchMenuManageScroll();
	
	// Also try after DOM is ready
	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', patchMenuManageScroll);
	} else {
		// DOM already loaded, try again after a short delay
		setTimeout(patchMenuManageScroll, 100);
	}
	
	// Patch on window resize with debounce to prevent errors
	let resizeTimeout;
	const originalResize = window.onresize;
	window.addEventListener('resize', function() {
		clearTimeout(resizeTimeout);
		resizeTimeout = setTimeout(function() {
			// Ensure menu is patched before calling manageScroll
			patchMenuManageScroll();
			if (window.Helpers && window.Helpers.mainMenu) {
				try {
					window.Helpers.mainMenu.manageScroll();
				} catch (error) {
					console.warn('Resize menu error handled:', error);
				}
			}
		}, 100);
	});
		...
		function initLanguageSwitcher() {
			...
			function setActiveLanguage(langCode) {
				...
				languageOptions.forEach(option => {
					...
					const span = option.querySelector('.text-secondary');
					if (span) {
						span.style.color = '';
				...
				const selectedOption = document.querySelector('.language-option[data-lang-code="' + langCode + '"]');
				if (selectedOption) {
					selectedOption.classList.add('active');
					selectedOption.style.backgroundColor = 'rgba(15, 121, 243, 0.1)';
					const span = selectedOption.querySelector('.text-secondary');
					if (span) {
						span.style.color = '#0f79f3';
			...
			languageOptions.forEach(option => {
				option.addEventListener('click', function(e) {
					...
					if (langCode) {
						...
						setTimeout(function() {
							const dropdownButton = document.querySelector('.language .btn[data-bs-toggle="dropdown"]');
